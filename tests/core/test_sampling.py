...
def test_sampling_rate():
    sr = nocte.core.sampling.SamplingRate(30_000)
...
def test_sampling_rate_rejects_invalid_rate(rate):
    with pytest.raises(ValueError):
        nocte.core.sampling.SamplingRate(rate)


def test_from_period_ms():
    sr = nocte.core.sampling.SamplingRate.from_period_ms(0.5)
...
def test_from_period_ms_rejects_invalid_period(period_ms):
    with pytest.raises(ValueError):
        nocte.core.sampling.SamplingRate.from_period_ms(period_ms)


def test_period_ms():
    sr = nocte.core.sampling.SamplingRate(2000)
...
def test_round_to_period_warns_with_description(caplog):
    sr = nocte.core.sampling.SamplingRate(1000)
