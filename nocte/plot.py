...
def _auto_select_tick_steps(ax, which="x") -> tuple[float, float]:
    ...
    sections = {
        ...
        ms(minutes=10): (ms(minutes=1), ms(minutes=0.5)),
...
def plot_wins_rectangle(
    ...
    **kwargs,
    ...
    for win_idx, x0, x1, cat in wins.wins[["start", "stop", by]].itertuples():
        ...
        rect = matplotlib.patches.Rectangle((x0, y0), **full_kwargs)
...
def _get_stack_extent(s, xcol, ycol):
    ...
    return (
        s.coords[xcol][0] * 1.5 - s.coords[xcol][1] * 0.5,
        s.coords[xcol][-1] * 1.5 - s.coords[xcol][-2] * 0.5,
        s.coords[ycol][0] * 1.5 - s.coords[ycol][1] * 0.5,
        s.coords[ycol][-1] * 1.5 - s.coords[ycol][-2] * 0.5,
    )
...
def add_desc(
    ...
    **kwargs,
    ...
    default_kwargs = dict(
        ...
        zorder=1e6,
        bbox=dict(
            facecolor=bkg_color, alpha=bkg_alpha, edgecolor=bkg_edgecolor, linewidth=0.5
        ),
    )
...
def darken_color(original_color_name, darken_factor):
    ...
    darkened_hsl_color = (
        hsl_color[0],
        hsl_color[1],
        np.clip(hsl_color[2] * darken_factor, 0, 1),
    )
...
def plot_df_as_im(
    ...
    **kwargs,
    ...
    """
    x_extent = df.index[[0, -1]] + np.diff(df.index)[[0, -1]] * np.array([-0.5, +0.5])
    y_extent = df.columns[[0, -1]] + np.diff(df.columns)[[0, -1]] * np.array(
        [-0.5, +0.5]
    )
...
def plot_violin_scatter(
    ...
    ax.fill_betweenx(
        ...
        x - density,
        x + density,
        edgecolor=shade_edgecolor,
        facecolor=shade_facecolor,
        alpha=shade_alpha,
