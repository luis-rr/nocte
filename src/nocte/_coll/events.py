...
from nocte._coll.windows import Win, Windows, WinPoint, _TimeWindowPairs
from nocte._core import point_process as _pp
...
from nocte._core.time import TimeScale, ms, scale_to_ms
...
class Events(HDFCollection[float]):
    ...
    def count_bins(self, bins: TimeArrayLike) -> pd.Series:
        ...
        """
        edges = _pp.as_bin_edges(bins)
        values = _pp.count_bins_many(
    ...
    def count_rolling(
        ...
        *,
        ...
        sample_times = _pp.sample_centers(
            within.time_at('start'),
            within.time_at('stop'),
            step,
            margin=window * 0.5,
        )
        values = _pp.count_rolling_many(
            self._point_processes(),
            sample_times,
            window,
        )[0]
    ...
    def extract_all(
        ...
        *,
        ...
        pairs = _TimeWindowPairs.from_times(
            wins,
            self._time,
        )

        matches = Matches.from_pairs(
            self,
            wins,
