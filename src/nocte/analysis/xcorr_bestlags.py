...
class XCorrBestLags:
    ...
    def plot_hist2d(
        ...
        **kwargs,
        ...
        for c in [c0, c1]:
            if c == 'lag':
                bins.append(
                    np.arange(self.reg['lag'].min() - 0.5, self.reg['lag'].max() + 1, 1)
                )
    ...
    def plot_hist1d_lags(
        ...
        if lag_bins is None:
            lag_bins = np.arange(
                self.reg['lag'].min() - 0.5, self.reg['lag'].max() + 1, 1
            )
    ...
    def plot_sel_min_xcorr(self, vmin, suptitle=''):
        f, axs = plt.subplots(
            nrows=3, sharex='all', constrained_layout=True, figsize=(3, 3)
        )
        f.suptitle(suptitle)
        ...
        ax = axs[0]
        self.sel_xcorr_range(vmin=vmin).plot_hist1d_lags(
            ax=ax, facecolor='r', clip_on=False
        )
        ax.set_xlabel('')

        ax = axs[1]
        self.sel_xcorr_range(vmax=vmin).plot_hist1d_lags(
            ax=ax, facecolor='k', clip_on=False
        )
        ax.set_xlabel('')

        ax = axs[2]
        self.plot_hist2d(
            'lag', 'xcorr', ax=ax, norm=matplotlib.colors.LogNorm(), clip_on=False
        )
        ax.axhline(vmin, color='r', linewidth=0.5, linestyle='--')
    ...
    def sel_xcorr_range_q(self, qmin=0.0, qmax=1.0):
...
def extract_best_lags_multi_exp(exp_xcorr, time_range, lag_range):
    ...
    for exp_desc, xcorr in exp_xcorr.items(pbar='best lags'):
        ...
        assert xcorr.coords['lag'].min() <= extended_valid_lags[0], (
            'lags extracted in window shorter than requested'
        )
        assert xcorr.coords['lag'].max() >= extended_valid_lags[1], (
            'lags extracted in window shorter than requested'
        )
    ...
    return DataDict(exp_xcorr.reg.loc[exp_best_lags.keys()], exp_best_lags)


def plot_dists_best_lags(
    exp_hists: pd.DataFrame,
    ylabel='prob',
    figsize=(3, 3),
    suptitle=None,
    sharey='none',
    ...
    for i, pair in enumerate(pairs):
        ...
        for exp_name, trace in traces.items():
            ax.plot(
                ...
                trace.values,
                label=exp_name,
                color='k' if not legend else None,
                linewidth=0.5,
            )
        ...
        if len(traces.columns) > 1:
            ...
            ax.plot(
                x,
                mean.values,
                color='r' if not legend else 'k',
                zorder=1e6,
                linestyle='--' if legend else '-',
            )
        ...
        if legend:
            ax.legend(
                loc='upper right', fontsize=4, ncol=1 if len(traces.columns) <= 8 else 2
            )
...
def compute_lags_best_xcorr(
    ...
    if cache_path.exists() and use_cache:
        ...
        exp_xcorr = reg.load_exp_xcorr_combs(
            exp_names, sliding_win=sliding_win, suffix=suffix
        )
...
def compute_dist_best_lags(exp_best_lags, min_xcorr_q=0.75) -> pd.DataFrame:
    ...
    hists = exp_best_lags.extract_df(extract_hist_best_lags)
