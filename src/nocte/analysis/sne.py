...
class SharpNegativeEvents(Events):
    ...
    def add_details(self, all_beta):
        ...
        copy.reg['ref_beta_local'] = pd.concat(
            [copy.sel(channel=ch).reg[f'ref_beta_ch{ch}'] for ch in all_beta.columns]
        ).sort_index()

        copy.reg['duration_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['duration'] > 0, 'duration'])
        )
        copy.reg['amplitude_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['amplitude'] < 0, 'amplitude'])
        )
        copy.reg['amplitude_filt_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['amplitude_filt'] < 0, 'amplitude_filt'])
        )
        copy.reg['speed_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['speed'] < 0, 'speed'])
        )

        copy.reg['ref_speed_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['ref_speed'] < 0, 'ref_speed'])
        )
        copy.reg['start_acc_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['start_acc'] < 0, 'start_acc'])
        )
        copy.reg['stop_acc_log'] = np.log10(
            np.abs(copy.reg.loc[copy.reg['stop_acc'] > 0, 'stop_acc'])
        )
        ...
        for col in [
            ...
            copy.reg[f'{col}_log'] = np.log10(
                np.abs(copy.reg.loc[copy.reg[col] > 0, col])
            )
    ...
    def assign_matches(
        ...
        for col in cols:
            copy.loc[matches.index, f'{name}_{col}'] = copy.loc[
                matches.values, col
            ].values
        ...
        if not quiet:
            ...
            print(
                f'matched {matched_count:,g}/{len(copy):,g} ({100 * matched_count / len(copy):.1f}%) events'
            )
    ...
    def get_match(self, match_col, name='match') -> pd.Series:
        ...
        if np.any(missing):
            print(
                f'Missing data for {np.count_nonzero(missing)}/{len(missing)} match ids'
            )
    ...
    def _get_simplified_channel_idx(self):
        mapping = (
            self.reg[['probe', 'channel']]
            .drop_duplicates()
            .sort_values(['probe', 'channel'])
            .copy()
        )
    ...
    @classmethod
    def load_matched_sns(
        cls, reg, exp_name, sn_suffix='_cdf', matching_suffix='_lax', drop_singles=False
    ):
        """
        ...
        matching: pd.DataFrame = pd.read_hdf(
            reg.get_path_matching(exp_name, suffix=matching_suffix)
        )
