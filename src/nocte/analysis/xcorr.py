...
def _cross_corr_nb(
    ...
    assert np.max(idcs_shifted[:, 1]) < len(signal1), (
        'must zero-pad array or crop windows'
    )
...
def valid_cross_corr(
    ...
    if signal.get_global_win().length < min_valid:
        raise ValueError(
            f'Signal must be at least {min_valid}ms. Given {signal.get_global_win().length}ms'
        )
    ...
    valid_wins = (0 <= (sliding_wins['start'] + np.min(lags_idx))) & (
