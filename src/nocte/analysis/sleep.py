...
FREQ_BANDS = pd.DataFrame.from_dict(
    ...
    orient='index',
    columns=['freq_min', 'freq_max', 'freq_idx'],
)
...
def band_power(
    traces: np.array, sampling_rate, bands, welch_ms=None, db=False, add_total=True
):
    ...
    for i, (name, freq_range) in enumerate(bands[['freq_min', 'freq_max']].iterrows()):
        mask = (freq_range['freq_min'] <= sample_frequences) & (
            sample_frequences <= freq_range['freq_max']
        )
    ...
    # total power
    if add_total:
        values[-1] = scipy.integrate.simpson(
            power_spectral_density, dx=frequency_resolution, axis=0
        )
...
def extract_power(
    traces: stacks.Stack, bands=FREQ_BANDS, welch_ms=None, add_total=True
):
    """
...
def extract_power_sliding(
    main: Stack,
    sliding_step_ms=1_000.0,
    sliding_win_len_ms=20_000.0,
    bands=FREQ_BANDS,
    welch_ms=None,
    pbar=None,
    add_total=True,
