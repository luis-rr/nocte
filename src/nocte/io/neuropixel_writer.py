...
def _float_microvolts_to_int16(
    data_microvolts: np.ndarray, im_ai_range_max_v: float, ap_gain_index: int
) -> np.ndarray:
    """
...
def write_ap_bin(
    ...
    with open(str(bin_path), 'wb') as f:
        for chunk in tqdm(chunks):
            ...
            chunk_i16 = _float_microvolts_to_int16(
                chunk_float, im_ai_range_max_v, ap_gain_index
            )
...
def write_ap_meta(
    ...
    imro_entries = ''.join(
        f'({ch} 0 0 {ap_gain_index} {ap_gain_index} 0)' for ch in range(n_channels)
    )
...
def store_neuropixels_data(
    ...
    if not np.allclose(channel_counts, channel_counts[0]):
        raise ValueError(
            f'Inconsistent channel count across chunks: {list(channel_counts)}'
        )
