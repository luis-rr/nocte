...
class Entry:
    ...
    def get_loader_simplified(self, area='', accept_non_interp=True):
        ...
        assert np.all(
            loader.channels.index.values == np.arange(len(loader.channels.index))
        )
...
class Registry(Collection):
    ...
    def _paths_patch(paths):
        ...
        alt_exist = (
            ...
            .reindex(alt.index, fill_value=False)
        )
    ...
    def get_all_events(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {exp_name: self.get_events(exp_name) for exp_name in self.experiment_names}
        )
    ...
    def collect_paths_power(
        self, band, sliding_win, sliding_step, areas=None, missing=False
    ):
        to_extract = []
        ...
        for exp_name in self.experiment_names:
            ...
            for idx in probe_idcs:
                ...
                results_path = self.get_path_power(
                    exp_name, band, idx, ch, sliding_win, sliding_step
                )
    ...
    def load_timestamps(self, col, rename='ref_time') -> nocte.core.events.Events:
        ...
        def parse_entry(string):
            ...
            try:
                ...
                raise ValueError(
                    f'Expected string format "description - Xd HH:MM:SS.MS" got "{string}"'
                )
    ...
    def load_area_beta(self, exp_name, simplify=True, **kwargs):
        ...
        traces = self.load_all_beta_norm(
            exp_name, simplify=False, add_max=False, **kwargs
        )
